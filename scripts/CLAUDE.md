...
`check_cmd`, `check_dir` and `check_file` fail the run. `check_cmd_optional` warns, for `perf`, `obsidian` and `zathura`, as does `check_optional`, a label plus command for the gh-stack extension and skill. `check_bash_version` requires bash 4.4 from a login shell via `env -i ... zsh -lc`, as direnv sees it. Plugins come from every `set -g @plugin` line in `~/.tmux.conf`, and `wezterm` and the Nerd Font are skipped on headless Linux.
