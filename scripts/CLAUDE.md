...
| `install_wezterm` | WezTerm — `brew install --cask wezterm@nightly` (macOS), the rolling `nightly` `.deb` from GitHub releases (Linux x86_64); skipped on ARM64 |
...
| `install_obsidian` | **Optional** (key `obsidian`) — Obsidian desktop app. `brew install --cask obsidian` (macOS), `obsidian_<ver>_amd64.deb` from GitHub releases (Linux x86_64), or the `obsidian-<ver>-arm64.tar.gz` unpacked to `~/.local/share/obsidian` (Linux ARM64, since upstream ships no ARM64 `.deb`); skipped on headless Linux. The Linux version comes from `obsidian_desktop_version`, not `github_latest_tag` |
...
Linux resolves the version through `obsidian_desktop_version` rather than `github_latest_tag`, because the repo's newest release is not necessarily a desktop one. Obsidian ships Android out of the same repository on its own cadence: v1.13.8 (21 August 2026) carried a single `Obsidian-1.13.8.apk` and nothing else, so `github_latest_tag` handed the download a version whose `.deb` and tarball do not exist and the step failed on a 404. `desktop-releases.json` on the repository's master branch is the manifest the app's own updater reads, and its `latestVersion` names the newest version with desktop builds — 1.13.7 at the time of writing. The `beta` block repeats the same key further down, which `grep -m1` skips. A missing or reshaped manifest fails the step with a message rather than falling back to the release list, since falling back is the bug.
...
macOS gets a LaunchAgent rather than a crontab entry. macOS still ships cron, but it runs under a sandbox needing Full Disk Access granted to `/usr/sbin/cron` by hand, and inherits a `PATH` with no Homebrew on it, so `git` is absent. The agent sets its own `PATH` and a `StartInterval` of 900 seconds. On Linux the entry is written under a `# obsync: managed by dotfiles install` marker; re-running filters out both the marker and any line matching `obsync.sh` before appending, so hand-written entries predating the marker are replaced rather than duplicated, and everything else in the crontab is passed through untouched. That filter used to sit inside `schedule_obsync_cron`, which `schedule_obsync` never reaches on a Mac, so the move to the LaunchAgent in 2b3086f left the old crontab line in place and both schedules ran, cron at :00/:15/:30/:45 and launchd at :09/:24/:39/:54 — 50 obsync runs in one day on the machine that had both. `obsync.sh` takes no lock and launchd resets its phase on every reboot or wake, so two runs can land together inside `pull --rebase --autostash` and `push`, which is what the `Failed to fetch` and aborted-rebase bursts in `backup.log` were attributed to; the hand-written line also carried no `>> ... 2>&1` redirect, so every run mailed its output locally and `/var/mail/benmandrew` reached 5.9 MB. The filter is now the shared helper `obsync_cron_stripped`, called by `schedule_obsync_cron` where two `grep -vF` calls were inlined and by a new `remove_obsync_cron` that installs the stripped result. `schedule_obsync_launchd` calls `remove_obsync_cron` after `launchctl bootstrap` succeeds rather than before, so a machine whose bootstrap failed keeps its crontab entry and stays scheduled by it, and `remove_obsync_cron` compares the stripped crontab against the original and returns without writing when they match, so a machine with no obsync line never rewrites its crontab.
